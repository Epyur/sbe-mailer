...
- **UI — фасад «LogicTEAM.Письма»** (как sbe-documents/sbe-requests/sbe-lims): топбар (создание,
  ...
  заголовок «номер — тема», чип даты, мета (автор · направление · статус синхронизации); клик
  по карточке открывает детали. Поиск и фильтры по дате/автору — как раньше, над списком карточек.

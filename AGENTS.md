...
### 2026-08-26 — v0.1.14 (безопасность: релевантный отбор контекста + санитизация PII в AI-генерации)
- **Ревью 3.5 (`plugins/secrev.md`)**: раньше `LlmGenerator` грузил в LLM ВСЮ локальную
  базу писем (авторы, темы, до 500 символов текста каждого письма) — PII уходила целиком.
  Теперь:
  - **Релевантный отбор вместо жёсткого фильтра** (`selectEmails`): ранжирование писем по
    совпадениям токенов запроса в `direction_name` (вес 3), `subject` (2), тексте (1);
    матч по подстроке ИЛИ общему корню ≥5 букв (дешёвый стеммер для русской морфологии —
    «гидроизоляцию» находит «гидроизоляция»). **Гарантия непустого контекста**: при нехватке
    прямых совпадений добираем письма наиболее релевантного направления, затем свежие; при
    полном отсутствии — последние письма + заметка. Контекст ограничен **20 письмами**.
  - **Санитизация PII**: email-адреса → `[email]`, телефоны RU → `[телефон]` (однозначно по
    формату); **имена маскируются только из известного списка** (авторы писем + локальный кэш
    контактов `yourbase/sbe_contacts/contacts_data.json`, подгружается best-effort из `main.ts`)
    целыми словами (не подстроками): «Иванов» → `[имя]`, но «Иваново» (топоним) не трогается.
    Падежные формы имён сознательно не маскируются (иначе задели бы топонимы/нарицательные).
  - Проверено: юнит-прогон чистых функций (маскировка/скоринг/фолбэк) + прогон на реальной
    базе 859 писем (по запросам «гидроизоляция кровли», «пожарная безопасность»,
    «поставка крепежа» — в топ-20 именно релевантные направления).
- Версия 0.1.13 → **0.1.14** (manifest + package.json). `npx tsc --noEmit` EXIT=0;
  `npm run build` OK.
